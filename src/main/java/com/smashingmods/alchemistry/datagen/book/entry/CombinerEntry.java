...
public class CombinerEntry extends EntryProvider {
    ...
    protected void generatePages() {
        ...
        pageText("The Chemical Combiner is a powered machine, perhaps the most important machine of all in Alchemistry. It allows you to combine elements, compounds, and occasionally other items to form new items. The combiner takes up to 4 input items similar to a 2x2 crafting grid, but unlike a crafting grid it allows each input slot to contain more than one item per craft.\\\\*From Professor Warren's notes: The Compactor is all well and good, but rather simplistic. I'm interested in what I can do with these components. With the focusing power of a diamond and the resistance of obsidian, I can use a piston to force these components into more complex items.*");
