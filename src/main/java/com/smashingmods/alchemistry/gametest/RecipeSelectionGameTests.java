 ...
 *
 * <p>Like {@link MachineGameTests} this class lives in {@code src/main} so the mod scan registers it for the
 * {@code gameTestServer} run, but the {@code jar} task excludes the {@code gametest} package so it never
 * ships. Each test pins {@code template = "loadsemptytemplate"} (the staged 3x3x3 air structure) with
 * {@code @PrefixGameTestTemplate(false)} so the id resolves un-prefixed to
 * {@code alchemistry:loadsemptytemplate}. Both machines default to 50 ticks per operation, well inside the
 * default 100-tick test timeout.</p>
 */
...
public class RecipeSelectionGameTests {
    ...
    public void combinerAutoPickDeterministic(GameTestHelper helper) {
        ...
        helper.succeedWhen(() -> {
            ...
            helper.assertTrue(recipe != null, "combiner never auto-picked a recipe");
            helper.assertTrue(FIRST_SAPLING_RECIPE.equals(recipe.getId()),
                    "auto-pick expected " + FIRST_SAPLING_RECIPE + ", got " + recipe.getId());
        });
    }
    ...
    @GameTest(template = "loadsemptytemplate")
    @PrefixGameTestTemplate(false)
    public void combinerSelectionSticksThroughProcessing(GameTestHelper helper) {
        CombinerBlockEntity combiner = placeCombiner(helper);
        ...
        helper.assertTrue(applied != null && JUNGLE_SAPLING_RECIPE.equals(applied.getId()),
                "selection did not apply: current recipe is " + (applied == null ? "null" : applied.getId()));
        ...
        helper.succeedWhen(() -> {
            ...
            helper.assertTrue(recipe != null && JUNGLE_SAPLING_RECIPE.equals(recipe.getId()),
                    "selection was replaced by " + (recipe == null ? "null" : recipe.getId()));
            ItemStack output = combiner.getOutputHandler().getStackInSlot(0);
            helper.assertFalse(output.isEmpty(), "combiner has not produced output yet");
            helper.assertTrue(ItemStack.isSameItemSameComponents(output, expected),
                    "selection was not honoured: expected " + expected.getItem() + ", produced " + output.getItem());
        });
    }
    ...
    @GameTest(template = "loadsemptytemplate")
    @PrefixGameTestTemplate(false)
    public void compactorSelectionSticksThroughProcessing(GameTestHelper helper) {
        ...
        long matching = RecipeRegistry.getCompactorRecipes(helper.getLevel()).stream()
                ...
                .count();
        helper.assertTrue(matching > 1, "ambiguous-input precondition failed: only " + matching + " compactor recipe(s) match " + input.getItem());
        ...
        helper.succeedWhen(() -> {
            ...
            helper.assertFalse(output.isEmpty(), "compactor has not produced output yet");
            helper.assertTrue(ItemStack.isSameItemSameComponents(output, expected),
                    "selection was not honoured: expected " + expected.getItem() + ", produced " + output.getItem());
        });
    }
    ...
    @GameTest(template = "loadsemptytemplate")
    @PrefixGameTestTemplate(false)
    public void combinerReselectMidProcessingTakesEffect(GameTestHelper helper) {
        CombinerBlockEntity combiner = placeCombiner(helper);
        ...
        helper.runAfterDelay(10, () -> {
            helper.assertTrue(combiner.getProgress() > 0, "machine should be mid-operation before re-selecting");
            ...
            helper.assertTrue(recipe != null && OAK_SAPLING_RECIPE.equals(recipe.getId()),
                    "re-selection did not take effect: current recipe is " + (recipe == null ? "null" : recipe.getId()));
            helper.assertTrue(combiner.getProgress() == 0, "re-selection must restart the operation");
        });
        ...
        helper.succeedWhen(() -> {
            CombinerRecipe recipe = combiner.getRecipe();
            helper.assertTrue(recipe != null && OAK_SAPLING_RECIPE.equals(recipe.getId()),
                    "re-selection was replaced by " + (recipe == null ? "null" : recipe.getId()));
            ItemStack output = combiner.getOutputHandler().getStackInSlot(0);
            helper.assertFalse(output.isEmpty(), "combiner has not produced output yet");
            helper.assertTrue(ItemStack.isSameItemSameComponents(output, expected),
                    "re-selection was not honoured: expected " + expected.getItem() + ", produced " + output.getItem());
        });
    }
    ...
    private static CombinerBlockEntity placeCombiner(GameTestHelper helper) {
        ...
        if (!(blockEntity instanceof CombinerBlockEntity combiner)) {
            helper.fail("expected a CombinerBlockEntity at " + MACHINE_POS + ", got "
    ...
    private static void assertAmbiguousCombinerInputs(GameTestHelper helper, CombinerBlockEntity combiner) {
        long matching = RecipeRegistry.getCombinerRecipes(helper.getLevel()).stream()
                ...
                .count();
        helper.assertTrue(matching > 1, "ambiguous-inputs precondition failed: only " + matching + " combiner recipe(s) match");
    }
}
