...
public class LiquifierRecipeProvider {
    ...
    private void liquifier(IngredientStack pInput, FluidStack pOutput) {
        ResourceLocation fluidId = Objects.requireNonNull(BuiltInRegistries.FLUID.getKey(pOutput.getFluid()));
        ResourceLocation fullId = ResourceLocation.fromNamespaceAndPath(Alchemistry.MODID, String.format("liquifier/%s", fluidId.getPath()));
        LiquifierRecipeBuilder.createRecipe(pInput, pOutput, fluidId)
