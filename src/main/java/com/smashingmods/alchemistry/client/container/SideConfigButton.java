...
class SideConfigButton extends AbstractWidget {
    ...
    SideConfigButton(SideModeConfigurationScreen parentScreen, int rasterX, int rasterY, @Nullable Direction side) {
        ...
        tooltip.add(new TranslatableComponent("alchemistry.container.sides." + (side == null ? "external" : side.getSerializedName())));
    ...
    private Component getCurrentModeComponent() {
        return new TranslatableComponent("alchemistry.container.sides.current")
                .append(new TranslatableComponent("alchemistry.container.sides.mode." + getCurrentMode().name().toLowerCase(Locale.ROOT)));
    }
