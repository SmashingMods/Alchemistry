...
public class AtomizerRecipeProvider {
    ...
    private void atomizer(FluidStack pInput, ItemStack pOutput) {
        ResourceLocation itemId = BuiltInRegistries.ITEM.getKey(pOutput.getItem());
        ResourceLocation recipeId = ResourceLocation.fromNamespaceAndPath(Alchemistry.MODID, String.format("atomizer/%s", Objects.requireNonNull(itemId).getPath()));
        AtomizerRecipeBuilder.createRecipe(pInput, pOutput, itemId)
